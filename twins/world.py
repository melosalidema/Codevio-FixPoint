...
class StripeTwin:
    ...
    def list_customers(self, tenant_id: str, email: str | None = None, name: str | None = None) -> list[Customer]:
        self._maybe_fail()
        out = []
    ...
    def refund(self, tenant_id: str, charge_id: str, amount_cents: int, idempotency_key: str) -> dict[str, Any]:
        self._maybe_fail()
...
class CrmTwin:
    ...
    def find_contacts(self, tenant_id: str, email: str | None = None, name: str | None = None) -> list[Contact]:
        out = []
...
class World:
    ...
    def reset(self, seed: dict[str, Any] | None = None) -> None:
        seed = seed or {}
        self.customers = {
            c["id"]: Customer(c["id"], c["email"], c["name"], c["tenant_id"]) for c in seed.get("customers", [])
        }
        self.charges = {
            c["id"]: Charge(
                c["id"],

...
from core.approval import verify_token
from core.canonical import action_hash, idempotency_key, tenant_of
...
def evaluate(
    ...
    if not contract["money"]:
        return GatewayDecision(decision=Decision.ALLOW, reason="low_risk_autonomous", action_hash=h, idempotency_key=key)
    ...
    if token is None:
        return GatewayDecision(
            ...
            reason="over_cap" if over_cap else "over_threshold",
            required_role=role,
            action_hash=h,
            idempotency_key=key,
            over_cap=over_cap,

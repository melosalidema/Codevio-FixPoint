...
@dataclass
class Resolution:
    """Server-assembled facts about the target of the request."""
...
def propose(facts: ParsedFacts, resolution: Resolution, ctx: RunContext) -> ProposedAction | None:
    """Deterministic planner: one action at a time, refuse if unclear.

    The planner has no execution authority; its output is a proposal that the
    Action Gateway then accepts, gates, or rejects.
    """
    ...
    params: dict[str, Any] = {
        ...
        if target["id"] in resolution.duplicate_charge_ids

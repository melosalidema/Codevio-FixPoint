...
from app.evals.scenarios import ACTOR, CAPABILITIES, SCENARIOS, TENANT
from app.providers.adapters import AdapterSet
from app.providers.world import ProviderError, World
from app.safety import gateway, webhooks
from app.safety.approval import issue_token
from app.safety.canonical import action_hash
from app.safety.models import Envelope, GatewayState, ProposedAction, RunContext
...
def run_scenario(spec: dict[str, Any]) -> dict[str, Any]:
    """Run one scenario against freshly seeded twins and grade the outcome.

    Every flow exercises the real safety code; nothing is mocked out or
    special-cased for the evaluation.
    """
    flow = spec["flow"]
    seed = spec["seed"]
    ...
    if flow == "run":
        ...
        observed = {
            "refund_cents": total_refunded(world),
            "no_mutation": total_refunded(world) == 0,
            "verified": report.verified,
        }
        ...
        for index in range(5):
            world = World(seed)
            session = RunSession(
                world, make_ctx(f"S13-{index}"), spec["request"], policy_file_id="doc_policy"
            )
            report = session.run()
        ...
        ctx = make_ctx("S5")
        state = GatewayState(
            allowed_charges={"ch_100", "ch_101"}, allowed_destinations={"ch_100", "ch_101", "cus_acme"}
        )
        ...
        observed = {
            ...
            "refunds": len(world.stripe._refund_keys),
        }
        ...
        # An approval issued for $42 must not authorize a $99.99 mutation.
        world = World(seed)
        ctx = make_ctx("S16")
        action_a = ProposedAction(
            tool="stripe.refund", params={"charge_id": "ch_100", "amount_cents": 4200, "reason": "duplicate"}
        )
        token = issue_token(ctx, action_hash(action_a), "team_lead", 4200)
        action_b = ProposedAction(
            tool="stripe.refund", params={"charge_id": "ch_100", "amount_cents": 9999, "reason": "duplicate"}
        )
...
def run_all() -> list[dict[str, Any]]:
    """Run the full matrix. A scenario error is a FAIL, never a crash."""
    results = []

...
from worker import planner as planner_module
from worker.planner import Resolution
from worker.quarantine_parser import parse
from worker.verifier import Intent, verify
...
class RunSession:
    ...
    def _investigate(self) -> None:
        ...
        self.audit("plan", {"facts": self.facts.model_dump(), "customers": len(customers), "charges": len(charges)})
    ...
    def _apply_decision(self, decision, token) -> None:
        ...
        if decision.decision == Decision.ALLOW:
            ...
            self._audit_log_append(LedgerType.UNSAFE_BLOCKED, {"action": self.proposed.model_dump(), "reason": decision.reason})
    ...
    def approve(self, role: str, approver_user_id: str) -> RunReport:
        ...
        if role != required and not (required == "team_lead" and role in {"finance", "finance_dual"}):
            ...
            self._audit_log_append(LedgerType.APPROVAL, {"approver": approver_user_id, "role": role, "result": "insufficient_role"})
        ...
        if self.artifact.separation_of_duties and approver_user_id == self.ctx.actor_user_id:
            self.artifact.status = "void"
            self._audit_log_append(LedgerType.APPROVAL, {"approver": approver_user_id, "result": "separation_of_duties_violation"})
        ...
        self._audit_log_append(LedgerType.APPROVAL, {"approver": approver_user_id, "role": role, "result": "approved"})
    ...
    def _execute(self, token) -> None:
        ...
        try:
            ...
            self._audit_log_append(LedgerType.TOOL_CALL, {"action": self.proposed.model_dump(), "error": str(error)})
            if error.status >= 500:
                result = self.adapter.execute(self.proposed, key)
            else:
                raise
    ...
    def _sync(self) -> None:
        ...
        if contact:
            try:
                self.world.crm.add_note(self.ctx.tenant_id, contact["id"], f"Fixpoint run {self.ctx.run_id}: remedy executed.")
                self._audit_log_append(LedgerType.TOOL_CALL, {"tool": "crm.note", "contact_id": contact["id"]})
            except ProviderError:
                pass
        try:
            self.world.slack_api.post_message(self.ctx.tenant_id, "#fixpoint-audit", f"run {self.ctx.run_id} completed")
        ...
        if email:
            self.world.gmail.create_draft(
                self.ctx.tenant_id,
                email,
                "Your refund has been processed",
                "We have processed your refund. This is a draft for human review.",
            )
    ...
    def _sync_reconciliation(self, note: str) -> None:
        ...
        if contact:
            try:
                self.world.crm.add_note(self.ctx.tenant_id, contact["id"], f"Fixpoint run {self.ctx.run_id}: {note}.")
            except ProviderError:
                pass

    def _finalize(self, expect_no_mutation: bool, outcome: str, executed: bool = False, reconcile: bool = False) -> RunReport:
        from core.schemas import LedgerType

        if expect_no_mutation:
            self.intent = Intent(expected_refund_charge=None, expected_refund_cents=0, expect_no_mutation=True)
        ...
        self._audit_log_append(LedgerType.REPORT, {"report": report.report_text, "verified": verification.passed})
        self.report = report
    ...
    def _build_claims(self, outcome: str, executed: bool) -> list[str]:
        claims = []
        if executed:
            claims.append(f"refund on {self.intent.expected_refund_charge} of {self.intent.expected_refund_cents} cents")
    ...
    def _report_text(self, outcome: str, verification) -> str:
        if outcome in {"unsafe_blocked", "approval_rejected_role", "approval_rejected_sod", "approval_voided"}:
            return "The requested action was refused by the safety gateway and logged as an unsafe-blocked event."

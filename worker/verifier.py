...
from core.schemas import CheckResult, RunContext, VerificationResult
from twins.world import SlackTwin, World
...
def verify(
    ...
    if intent.expect_no_mutation:
        ...
        checks.append(
            CheckResult(
                ...
                passed=ok,
                detail=f"charge {intent.expected_refund_charge} refunded {getattr(charge, 'refunded_cents', 'missing')}"
    ...
    bad_channels = [m for m in world.slack if m["tenant_id"] == tenant and m["channel"] not in SlackTwin.ALLOWED_CHANNELS]
    checks.append(
        CheckResult(

...
from core.schemas import ParsedFacts
...
def parse(text: str) -> ParsedFacts:
    injection_flags = [label for pattern, label in INJECTION_PATTERNS if re.search(pattern, text, re.IGNORECASE | re.MULTILINE)]
    amount = None
